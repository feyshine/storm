...
public class WxButtonBiz {
	...
	private List<com.cn.hnust.wx.api.req.entity.ComplexButton> getServerButtons() {
		...
		String url = String.format(Config.find_menu_url,
				TokenThread.accessToken.getAccessToken());
		String result = menuHelper.getHttpsResponse(url, "GET");
		List<com.cn.hnust.wx.api.req.entity.ComplexButton> list = ParseMenu
				.parseComplexButton(result);
		return list;
	}
	 ...
	 * @param list
	 */
	private void saveParentButtons(
	 ...
	 * @param parentBtn
	 */
	private void saveChildrenButton(
			com.cn.hnust.wx.api.req.entity.ComplexButton wxParentBtn,
			Button parentBtn) {
	...
	public static int creatWxButtons(List<com.cn.hnust.pojo.Button> parents) {
		NetWorkHelper menuHelper = new NetWorkHelper();
		String url = String.format(Config.creat_menu_url,
				TokenThread.accessToken.getAccessToken());
		String jsonMenu = CreateMenuFactory.creatMenu(parents);
		L.i("jsonMenu", jsonMenu);
		String result = menuHelper.postHttpsResponse(url, jsonMenu);
		ResponseResult menuresult = JSON.parseObject(result,
				ResponseResult.class);
		if (menuresult == null) {
			return WX_RESPONSE_NULL;
		}
		if (menuresult.getErrcode() != null
				&& menuresult.getErrcode().equals("0")) {
	 ...
	 * 删除自定义菜单
	 */
	public static int deleteWxButtons() {
		NetWorkHelper menuHelper = new NetWorkHelper();
		String url = String.format(Config.delete_menu_url,
				TokenThread.accessToken.getAccessToken());
		String result = menuHelper.getHttpsResponse(url, "GET");
		ResponseResult menuresult = JSON.parseObject(result,
				ResponseResult.class);
